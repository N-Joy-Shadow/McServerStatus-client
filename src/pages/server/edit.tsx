...
const ServerEdit = () => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_BASE_API_URL}/status/${name}`).then((x) =>{
      console.log(x)
    })
  },[])
  ...
  const onSubmit = async (data: any) => {
    ...
    await axios.put(`${import.meta.env.VITE_BASE_API_URL}`,s_data).then((x) => {
      if(x.data.success){
        enqueueSnackbar("성공!", { variant : "Toast",toastType : ToastEnum.success})
